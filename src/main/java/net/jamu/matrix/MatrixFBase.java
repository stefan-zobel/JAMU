...
public abstract class MatrixFBase extends DimensionsBase implements MatrixF {
    ...
    public MatrixF setSubmatrixInplace(int r0, int c0, MatrixF B, int rb0, int cb0, int rb1, int cb1) {
        ...
        if (B instanceof MatrixFView) {
            // a view may overlap this matrix, so copy its region first
            MatrixF region = B.submatrix(rb0, cb0, rb1, cb1, Matrices.createF(rb1 - rb0 + 1, cb1 - cb0 + 1), 0, 0);
    ...
    private float maxFiniteAbs() {
        ...
        float[] _a = a;
        for (int i = 0; i < _a.length; ++i) {
            float x = Math.abs(_a[i]);
            if (x > max && x != Float.POSITIVE_INFINITY) {
                max = x;
            }
        }
        return max;
