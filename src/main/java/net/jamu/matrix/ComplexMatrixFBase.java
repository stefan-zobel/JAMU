...
public abstract class ComplexMatrixFBase extends DimensionsBase implements ComplexMatrixF {
    ...
    public ComplexMatrixF scaleInplace(float alphar, float alphai) {
        ...
        for (int i = 0; i < _a.length; i += 2) {
            float ai = _a[i];
            float aip1 = _a[i + 1]; // codeql[java/index-out-of-bounds]
    ...
    public ComplexMatrixF addInplace(float alphar, float alphai, ComplexMatrixF B) {
        ...
        for (int i = 0; i < _b.length; i += 2) {
            float bi = _b[i];
            float bip1 = _b[i + 1]; // codeql[java/index-out-of-bounds]
    ...
    public float normMaxAbs() {
        ...
        for (int i = 0; i < _a.length; i += 2) {
            float re = _a[i];
            float im = _a[i + 1]; // codeql[java/index-out-of-bounds]
    ...
    public ComplexMatrixF zeroizeSubEpsilonRelativeInplace(int k) {
        ...
        for (int i = 0; i < _a.length; i += 2) {
            ...
            if (abs <= threshold) {
                _a[i] = 0.0f;
                _a[i + 1] = 0.0f; // codeql[java/index-out-of-bounds]
            }
        }
        return this;
    }
    ...
    public ComplexMatrixF appendMatrix(ComplexMatrixF matrix) {
        ...
        float[] _ab = new float[2 * (rows * colsNew)];
        System.arraycopy(_a, 0, _ab, 0, _a.length);
        System.arraycopy(_b, 0, _ab, _a.length, _b.length);
    ...
    public ComplexMatrixF timesMany(ComplexMatrixF m, ComplexMatrixF... matrices) {
        ...
        ArrayList<ComplexMatrixF> chain = MatrixChain.buildList(this, m, matrices);
     ...
     * Cormen et al.
     */
    private ComplexMatrixF multiplyMany(int i, int j, ArrayList<ComplexMatrixF> chain, UPInts splits) {
        if (i == j) {
            // base case
            return chain.get(i);
        }
        int k = splits.get(i, j);
        ComplexMatrixF X = multiplyMany(i, k, chain, splits);
        ComplexMatrixF Y = multiplyMany(k + 1, j, chain, splits);
    ...
    public ComplexMatrixF abs() {
        ...
        for (int i = 0; i < b_.length; i += 2) {
            float re = b_[i];
            float im = b_[i + 1]; // codeql[java/index-out-of-bounds]
            // nano-optimize
            if (im == 0.0f) {
                b_[i] = Math.abs(re);
                b_[i + 1] = 0.0f; // codeql[java/index-out-of-bounds]
            } else {
