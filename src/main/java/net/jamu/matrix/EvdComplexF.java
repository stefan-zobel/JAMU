...
public final class EvdComplexF {
    ...
    /* package */ EvdComplexF(ComplexMatrixF A, boolean full) {
        if (!A.isSquareMatrix()) {
            throw new IllegalArgumentException("EVD only works for square matrices");
        }
        int n = A.numRows();
        rightEVec = full ? TEigJob.ALL : TEigJob.VALUES_ONLY;
        eigenVectors = full ? new SimpleComplexMatrixF(n, n) : null;
        computeEvdInplace(A);
    }

    private void computeEvdInplace(ComplexMatrixF A) {
        // note that A need not be copied in the complex case
        int n = A.numRows();
        int ld = Math.max(1, n);
        float[] complexEigenVals = new float[2 * n];
        PlainLapack.cgeev(Lapack.getInstance(), leftEVec, rightEVec, n, A.getArrayUnsafe(), ld, complexEigenVals,
                new float[0], ld, hasEigenvectors() ? eigenVectors.getArrayUnsafe() : new float[0], ld);
