...
/**
 * QR decomposition of a single-precision complex m-by-n, with {@code m >= n},
...
public final class QrdComplexF {
    ...
    /* package */ QrdComplexF(ComplexMatrixF A) {
        ...
        Q = new SimpleComplexMatrixF(A.numRows(), A.numColumns());
        R = new SimpleComplexMatrixF(A.numColumns(), A.numColumns());
        computeQrdInplace(A);
    }

    private void computeQrdInplace(ComplexMatrixF A) {
        ComplexMatrixF AA = A.copy();
        int m = AA.numRows();
        int n = AA.numColumns();
        int k = Math.min(m, n);
        int lda = Math.max(1, m);
        float[] tau = new float[2 * k];
        Lapack la = Lapack.getInstance();
        // compute the QR factorization
        PlainLapack.cgeqrf(la, m, n, AA.getArrayUnsafe(), lda, tau);
        R.setInplaceUpperTrapezoidal(AA);
        // compute the elements of Q explicitly
        PlainLapack.cungqr(la, m, n, k, AA.getArrayUnsafe(), lda, tau);
